...
module Control.Monad.Par.DistTrace (
   Trace(..), Sched(..), Par(..),
   IVar(..), IVarContents(..),
   sched,
...
data Trace = forall a . Get (IVar a) (a -> Trace)
           ...
           | Fork Trace Trace
           | LongFork Trace Trace
           | Done
           | Yield Trace

-- | The main scheduler loop.
sched :: Bool -> Sched a -> Trace -> ProcessM ()
sched _doSync queue t = do t <- tryReceive t
                           loop t
 where 
  loop :: Trace -> ProcessM ()
  loop t = case t of
...
-- | Attempt to receive a message from completed work on a
-- potentially-remote node, pushing a 'Put' into the front of the
-- trace if a message is received.
tryReceive :: Trace -> ProcessM Trace
tryReceive = undefined

-- | Process the next item on the work queue or, failing that, go into
--   work-stealing mode.
reschedule :: Sched a -> ProcessM ()
...
pushWork :: Sched a -> Trace -> ProcessM ()
...
data Sched a = Sched
    { no       :: {-# UNPACK #-} !Int,
      ...
      remIvars :: IORef (Map.Map Int (a -> ProcessM Trace)),
...
pollIVar :: IVar a -> ProcessM (Maybe a)
pollIVar (IVar ref) = 
...
runPar_internal _doSync x = remoteInit Nothing [id] $ \_ -> do
   workpools <- replicateM numCapabilities $ newIORef []
   idle <- newIORef []
   remIvars <- newIORef $ Map.empty
   let states = [ Sched { no=x, workpool=wp, idle, remIvars, scheds=states }
